...
public class OnlyInputPortsConnectedValidationRule : ISchemeValidationRule
{
    ...
    {
        var invalidWiresGroups = context.PreprocessedScheme.InvalidConnection.Where(x => x.OutputPorts.Count == 0 && x.InputPorts.Count > 0).Select(x => x.WiresGroup).ToList();
    ...
    public class Result : SchemeValidationResult
    {
        public Result(ValidationContext context, List<IWiresGroup> invalidWiresGroups, bool isValid, ValidationRuleLevel level) : base(isValid, level)
        {
            ...
            Messages = InvalidWiresGroups.Select(x => new OutputMessageViewModel("Только входные порты объединены в цепь.", Level, new WiresMessageSource(context.Scheme, x.Wires)));
        }
